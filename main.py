...
class Form(StatesGroup):
    ...
    # Free Credit Links FSM
    waiting_for_genfree_count = State()
    waiting_for_genfree_credits = State()
    waiting_for_shortlinks_file = State()
    waiting_for_alert_threshold = State()
            ...
            if feature_enabled == '1':
                ...
                if short_link:
                    text = (
                        ...
                        "💰 <b>Free Credits</b> paane ke liye neeche button dabayein aur ad/video dekhein.\n\n"
                    ...
                    await message.reply(
                        ...
                        "Abhi koi free credit offer available nahi hai. Kripya credits buy karein.",
...
async def admin_owner(callback: types.CallbackQuery):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🔗 Free Credit Links", callback_data="admin_free_links_menu")],
        [InlineKeyboardButton(text="🔙 Back to Admin", callback_data="admin_back")]
    ]
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))

# ---------- FREE CREDIT LINKS (INLINE PANEL) ----------
@dp.callback_query(F.data == "admin_free_links_menu")
async def free_links_main_menu(callback: types.CallbackQuery):
    if not await is_user_owner(callback.from_user.id):
        await callback.answer("❌ Sirf Owner access kar sakta hai!", show_alert=True)
        return

    stats = await get_pool_stats()
    feature_enabled = await get_free_credit_setting('feature_enabled')
    status_emoji = "🟢" if feature_enabled == '1' else "🔴"

    text = f"""
🔗 <b>Free Credit Links Management</b>

{status_emoji} <b>Status:</b> {'Enabled' if feature_enabled == '1' else 'Disabled'}
📊 <b>Pool Stats:</b>
...
⚙️ <i>Kya karna chahte ho?</i>
"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🆕 Generate New Tokens", callback_data="free_generate_start")],
        [InlineKeyboardButton(text="📤 Upload Short Links", callback_data="free_upload_start")],
        [InlineKeyboardButton(text="📊 View Detailed Stats", callback_data="free_view_stats")],
        [InlineKeyboardButton(text="⚙️ Settings", callback_data="free_settings_menu")],
        [InlineKeyboardButton(text="🔙 Back to Owner Menu", callback_data="admin_owner")]
    ])
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "free_view_stats")
async def free_view_stats(callback: types.CallbackQuery):
    stats = await get_pool_stats()
    text = f"""
📊 <b>Detailed Pool Statistics</b>

🆕 <b>Pending (Short link nahi diya):</b> {stats['pending']}
✅ <b>Available (Use hone ke liye ready):</b> {stats['available']}
⏳ <b>Reserved (Kisi user ko diya hua):</b> {stats['reserved']}
✔️ <b>Used (Claim ho chuke):</b> {stats['used']}

📌 <b>Total Links in Pool:</b> {sum(stats.values())}
"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Back", callback_data="admin_free_links_menu")]
    ])
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "free_settings_menu")
async def free_settings_menu(callback: types.CallbackQuery):
    feature_enabled = await get_free_credit_setting('feature_enabled')
    threshold = await get_free_credit_setting('alert_threshold') or "5"

    text = f"""
⚙️ <b>Free Credit Settings</b>

🔘 <b>Feature Status:</b> {'🟢 ON' if feature_enabled == '1' else '🔴 OFF'}
🔔 <b>Alert Threshold:</b> {threshold} links

<i>Kya change karna hai?</i>
"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔛 Toggle ON/OFF", callback_data="free_toggle_feature")],
        [InlineKeyboardButton(text="🔔 Set Alert Threshold", callback_data="free_set_threshold")],
        [InlineKeyboardButton(text="🔙 Back", callback_data="admin_free_links_menu")]
    ])
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == "free_toggle_feature")
async def free_toggle_feature(callback: types.CallbackQuery):
    current = await get_free_credit_setting('feature_enabled')
    new_val = '0' if current == '1' else '1'
    await set_free_credit_setting('feature_enabled', new_val)
    await callback.answer(f"Feature {'Enabled' if new_val=='1' else 'Disabled'}!", show_alert=True)
    await free_settings_menu(callback)

@dp.callback_query(F.data == "free_set_threshold")
async def free_set_threshold_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "🔔 <b>Alert Threshold Set Karein</b>\n\n"
        "Jab available links is number se kam ho jayenge to bot aapko alert karega.\n"
        "Koi number bhejo (e.g., 5):",
        parse_mode="HTML"
    )
    await state.set_state(Form.waiting_for_alert_threshold)
    await callback.answer()

@dp.message(Form.waiting_for_alert_threshold)
async def free_threshold_handler(message: types.Message, state: FSMContext):
    try:
        val = int(message.text.strip())
        if val < 1:
            raise ValueError
    except:
        await message.answer("❌ Sirf positive number daalo (e.g., 5).")
        return
    await set_free_credit_setting('alert_threshold', val)
    await message.answer(f"✅ Alert threshold <b>{val}</b> set kar diya gaya.", parse_mode="HTML")
    await state.clear()

# --- Generate Tokens Flow (FSM) ---
@dp.callback_query(F.data == "free_generate_start")
async def free_generate_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "🆕 <b>Naye Tokens Generate Karein</b>\n\n"
        "Kitne tokens chahiye? (Max 500)\n"
        "Sirf number bhejo:",
        parse_mode="HTML"
    )
    await state.set_state(Form.waiting_for_genfree_count)
    await callback.answer()

@dp.message(Form.waiting_for_genfree_count)
async def free_gen_count_handler(message: types.Message, state: FSMContext):
    try:
        count = int(message.text.strip())
        if count <= 0 or count > 500:
            raise ValueError
    except:
        await message.answer("❌ Sahi number daalo (1-500).")
        return
    await state.update_data(genfree_count=count)
    await message.answer(
        f"💰 <b>Har token par kitne credits dena hai?</b>\n"
        "Sirf number bhejo (e.g., 5):",
        parse_mode="HTML"
    )
    await state.set_state(Form.waiting_for_genfree_credits)

@dp.message(Form.waiting_for_genfree_credits)
async def free_gen_credits_handler(message: types.Message, state: FSMContext):
    try:
        credits = int(message.text.strip())
        if credits <= 0:
            raise ValueError
    except:
        await message.answer("❌ Positive number daalo.")
        return

    data = await state.get_data()
    count = data['genfree_count']
    await state.clear()

    status_msg = await message.answer("🔄 Tokens generate ho rahe hain...")
    tokens = await generate_free_tokens(count, credits, bot)

    if not tokens:
        await status_msg.edit_text("❌ Kuch error aa gaya.")
        return
    ...
    await message.answer_document(
        FSInputFile(txt_path, filename=f"free_tokens_{count}.txt"),
        caption=f"✅ <b>{count} tokens generate ho gaye!</b>\n\n"
                f"💰 Har token ke credits: <b>{credits}</b>\n\n"
                f"📌 <b>Ab kya karna hai:</b>\n"
                f"1. Ye file kholo.\n"
                f"2. Har 'Deep Link' ko VPLink par short karo.\n"
                f"3. Short links ko <b>Token,ShortLink</b> format me save karo.\n"
                f"4. Fir <b>Upload Short Links</b> button use karo.",
        parse_mode="HTML"
    )
    os.unlink(txt_path)

# --- Upload Short Links Flow (FSM) ---
@dp.callback_query(F.data == "free_upload_start")
async def free_upload_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "📤 <b>Short Links Upload Karein</b>\n\n"
        "Ab aapne jo short links banaye hain, unhe ek <b>.txt file</b> ke roop me bhejein.\n\n"
        "📄 <b>File Format (Har line):</b>\n"
        "<code>TOKEN,SHORT_LINK</code>\n\n"
        "Ya comma separated text message bhej sakte ho.",
        parse_mode="HTML"
    )
    await state.set_state(Form.waiting_for_shortlinks_file)
    await callback.answer()

@dp.message(Form.waiting_for_shortlinks_file)
async def free_upload_handler(message: types.Message, state: FSMContext):
    mapping = {}
    if message.document:
        doc = message.document
        file = await bot.get_file(doc.file_id)
        dest = tempfile.NamedTemporaryFile(delete=False, suffix='.txt')
        await bot.download_file(file.file_path, dest.name)
        try:
            with open(dest.name, 'r', encoding='utf-8') as f:
                lines = f.readlines()
        except:
            await message.answer("❌ File padhne me error.")
            os.unlink(dest.name)
            await state.clear()
            return
        os.unlink(dest.name)
    ...
    if not mapping:
        await message.answer("❌ Koi valid pair nahi mila. Format: TOKEN,SHORT_LINK")
        await state.clear()
        return

    success = await bulk_update_short_links(mapping)
    if success:
        stats = await get_pool_stats()
        await message.answer(
            f"✅ <b>{len(mapping)} short links upload ho gaye!</b>\n\n"
            f"📊 Ab pool me <b>{stats['available']}</b> links available hain.",
            parse_mode="HTML"
        )
        # Alert check
        threshold = await get_free_credit_setting('alert_threshold')
        if threshold and stats['available'] < int(threshold):
            await bot.send_message(
                message.from_user.id,
                f"⚠️ <b>Alert:</b> Available links sirf {stats['available']} bache hain. Jaldi naye links daalo!",
                parse_mode="HTML"
            )
    else:
        await message.answer("❌ Upload karne me error aaya.")
    await state.clear()
...
# --- Auto-detect 10-digit mobile number ---
...
async def main():
    ...
    scheduler.add_job(cleanup_expired_reserved, 'interval', minutes=15, kwargs={'minutes': 30})
    scheduler.start()
