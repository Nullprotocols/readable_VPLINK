...
async def init_db():
    ...
    try:
        ...
        """)
        # Free Credit Link Pool Tables
        await db.execute("""
            CREATE TABLE IF NOT EXISTS free_link_pool (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        """)
        await db.execute("""
            CREATE TABLE IF NOT EXISTS free_credit_settings (
                key TEXT PRIMARY KEY,
                value TEXT
            )
        """)
        await db.execute("""
            INSERT OR IGNORE INTO free_credit_settings (key, value)
            VALUES 
                ('alert_threshold', '5'),
                ('feature_enabled', '1')
        """)
...
# ---------- Free Credit Link Pool System ----------
async def get_free_credit_setting(key):
    db = await get_db()
    try:
        async with db.execute("SELECT value FROM free_credit_settings WHERE key = ?", (key,)) as cursor:
            row = await cursor.fetchone()
            return row['value'] if row else None
    finally:
        await db.close()

async def set_free_credit_setting(key, value):
    db = await get_db()
    try:
        await db.execute("INSERT OR REPLACE INTO free_credit_settings (key, value) VALUES (?, ?)", (key, str(value)))
        await db.commit()
    finally:
        await db.close()

async def generate_free_tokens(count, credits, bot):
    """Generate unique tokens and return list of (token, deep_link). bot parameter is required."""
    db = await get_db()
    tokens = []
    try:
        ...
        for _ in range(count):
            while True:
                ...
                async with db.execute("SELECT 1 FROM free_link_pool WHERE token = ?", (token,)) as cur:
                    if not await cur.fetchone():
                        break
            created_at = datetime.now().isoformat()
            await db.execute("""
                INSERT INTO free_link_pool (token, credits, status, created_at)
                VALUES (?, ?, 'pending', ?)
            """, (token, credits, created_at))
            deep_link = f"https://t.me/{bot_username}?start=free_{token}"
            tokens.append((token, deep_link))
        await db.commit()
        return tokens
    finally:
        await db.close()

async def bulk_update_short_links(mapping):
    """mapping: dict {token: short_link}"""
    db = await get_db()
    try:
        for token, short_link in mapping.items():
            await db.execute("""
                UPDATE free_link_pool 
                SET short_link = ?, status = 'available'
...
async def get_available_link_for_user(user_id, chat_id=None, message_id=None):
    ...
    try:
        async with db.execute("""
            SELECT id, token, short_link FROM free_link_pool
            WHERE status = 'available' AND short_link IS NOT NULL
            LIMIT 1
        """) as cursor:
            row = await cursor.fetchone()
            if not row:
...
async def verify_and_claim_free_token(token, user_id):
    db = await get_db()
    try:
        async with db.execute("""
...
async def get_pool_stats():
    ...
    try:
        stats = {}
        for status in ['pending', 'available', 'reserved', 'used']:
            async with db.execute("SELECT COUNT(*) FROM free_link_pool WHERE status = ?", (status,)) as cur:
...
async def cleanup_expired_reserved(minutes=30):
    db = await get_db()
    try:
        cutoff = (datetime.now() - timedelta(minutes=minutes)).isoformat()
        await db.execute("""
            UPDATE free_link_pool
            SET status = 'available', reserved_for = NULL, reserved_at = NULL
